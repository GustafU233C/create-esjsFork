...
export const inExistingGitTree = (): boolean => {
  ...
  try {
    ...
    console.info(`${yellow('❗')} Se detectó un repositorio git existente, no se creará uno nuevo`);
    isInTree = true;
  } catch (_err: unknown) {
    console.info(`${green('✔')} Se inicializó un nuevo repositorio git`);
  }
  return isInTree;
};
...
export const commitAllFiles = (): boolean => {
  let wasSuccess = false;
  let createEsJSVersion = null;
  try {
    createEsJSVersion = ` v${getPkgVersion()}`;
  } catch (err: unknown) {
  ...
  try {
    ...
    // Ahora en español:
    const commitMessage = `inicializado con create-esjs${createEsJSVersion ?? ''}`;
